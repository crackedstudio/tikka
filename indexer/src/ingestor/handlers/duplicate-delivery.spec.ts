...
function createInMemoryDb(initial: DurableState) {
  ...
  const manager = {
      ...
      builder.execute = jest.fn(async () => {
        ...
        if (ctx.op === 'update' && ctx.into === RaffleEntity && ctx.set) {
          ...
          if (ctx.set.status !== undefined) {
            ...
            const whereSql = ctx.where?.sql ?? '';
            const blocked =
              (whereSql.includes('status != :finalized') &&
                raffle.status === RaffleStatus.FINALIZED) ||
              (whereSql.includes('status != :cancelled') &&
                raffle.status === RaffleStatus.CANCELLED) ||
              (whereSql.includes('status = :open') &&
                ctx.where?.params.open !== undefined &&
                raffle.status !== RaffleStatus.OPEN);
            if (blocked) return { affected: 0 };
        ...
        if (ctx.op === 'update' && ctx.into === TicketEntity && ctx.set) {
          ...
          const ticket = state.tickets.find((t) => t.id === ticketId && t.raffleId === raffleId);
        ...
        if (ctx.op === 'update' && ctx.into === UserEntity && ctx.set) {
          ...
          if (ctx.set.totalTicketsBought) {
            ...
            user.totalTicketsBought = (user.totalTicketsBought ?? 0) + Number(match?.[1] ?? 0);
          }
          ...
          if (ctx.set.totalPrizeXlm) {
            ...
            user.totalPrizeXlm = (BigInt(user.totalPrizeXlm ?? '0') + add).toString();
          }
          return { affected: 1 };
        }
    ...
    findOne: jest.fn(
      async (entity: unknown, opts: { where: Record<string, unknown>; select?: string[] }) => {
    ...
    query: jest.fn(async (sql: string, params: unknown[] = []) => {
      ...
      if (sql.includes('FROM tickets') && sql.includes('purchase_tx_hash !=')) {
        ...
        const prior = state.tickets.filter(
          (t) => t.owner === owner && t.raffleId === raffleId && t.purchaseTxHash !== txHash,
        );
...
describe('handler duplicate-delivery idempotency', () => {
  ...
  it('DrawTriggeredHandler: duplicate delivery leaves an identical DRAWING raffle', async () => {
    const event: DomainEvent = {
      type: 'DrawTriggered',
      schemaVersion: 1,
      raffle_id: 1,
    ...
    const raw = makeRawIngestionEvent('DrawTriggered', {
      id: mockTxHash('draw'),
      ledger: 900,
    });
    const initial: DurableState = {
    ...
    const { once, twice } = await deliverTwice(event, raw, initial);
    expect(snapshotState(twice)).toEqual(snapshotState(once));
    expect(once.raffles[0].status).toBe(RaffleStatus.DRAWING);
    expect(once.events).toHaveLength(1);
    expect(once.events[0].eventType).toBe('DrawTriggered');
  });

  it.each([RaffleStatus.FINALIZED, RaffleStatus.CANCELLED])(
    'DrawTriggeredHandler: does not reopen a %s raffle',
    async (terminalStatus) => {
      ...
      const raw = makeRawIngestionEvent('DrawTriggered', {
        id: mockTxHash(`draw-after-${terminalStatus}`),
        ledger: 900,
      });
      const initial: DurableState = {
        raffles: [
      ...
      const { once, twice } = await deliverTwice(event, raw, initial);
      expect(once.raffles[0].status).toBe(terminalStatus);
      expect(twice.raffles[0].status).toBe(terminalStatus);
    },
  );
