...
`event-parser-v2.service.spec.ts`, and structurally in
...
4. Route it in `IngestionDispatcherService.applyEvent` (and the
   `eventNeedsDatabase` / `getHandlerName` switches).
5. Add a decode test to `event-parser-v2.service.spec.ts` and a fixture to
   `handlers/event-coverage.spec.ts`.
