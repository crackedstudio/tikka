...
| `indexer_dlq_unreplayed_events`                         | Gauge           | Emitted     | (none)                 | Current total of unreplayed DLQ entries; refreshed at startup, enqueue, and replay                                                                                                                                                                                                                                                                 |
| `indexer_dlq_oldest_unreplayed_event_timestamp_seconds` | Gauge           | Emitted     | (none)                 | Unix timestamp of the oldest unreplayed entry; use `time() - metric` for age                                                                                                                                                                                                                                                                       |
...
| `IndexerDlqDepthHigh`        | `indexer_dlq_unreplayed_events >= 10` for 5m                | warning  |
| `IndexerDlqGrowthRateHigh`   | `deriv(indexer_dlq_unreplayed_events[15m]) > 0.0037` for 5m | critical |
| `IndexerDlqOldestEventStale` | oldest unreplayed entry older than 1h for 5m                | critical |
