...
This document tracks the TypeScript strict-mode migration for the NestJS
packages (backend, oracle, indexer, sdk). The shared `tsconfig.base.json`
already enables `strict: true`; the remaining work is to resolve package-level
compiler errors.

## Status

- ✅ Shared `tsconfig.base.json` enables `strict: true`, and NestJS package
  tsconfigs extend it
- ⏳ Package-level errors remain; current compiler counts are listed below
- ⚠️ Backend has no `typecheck` script, so the root `pnpm typecheck` workflow
  does not check it
...
### Phase 1: Assessment (BASELINE CAPTURED)

From the repository root, run the configured package checks. Turbo builds
package dependencies before checking them:

```bash
pnpm typecheck
```

The backend currently has no `typecheck` script and is skipped by that command.
Run its compiler directly to include it in the assessment:
...
The root typecheck workflow still skips the backend until a `typecheck` script
is added there; use the direct backend compiler command from Phase 1.
...
## Current Compiler Error Counts

Measured on 2026-09-25 with the package TypeScript checks after workspace
dependencies were built:

| Package | Errors | Notes                                                                                      |
| ------- | -----: | ------------------------------------------------------------------------------------------ |
| SDK     |      0 | Passes the configured typecheck                                                            |
| Indexer |      0 | Passes the configured typecheck                                                            |
| Oracle  |     76 | Strict-mode and other compiler errors remain                                               |
| Backend |      1 | TS1010 parse error in `src/queues/webhook-delivery.worker.ts`; no package typecheck script |

The backend count is from a direct compiler run. Its parse error prevents a complete strict-mode assessment until the syntax issue is fixed.
