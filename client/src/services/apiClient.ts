...
import { getDemoApiResponse } from '../demo/demo-api';
...
export async function apiRequest<T = unknown>(
  ...
  const { requiresAuth = false, headers = {}, signal, ...fetchOptions } = options;

  const url = endpoint.startsWith('http') ? endpoint : `${API_CONFIG.baseUrl}${endpoint}`;

  if (import.meta.env.VITE_DEMO_MODE === 'true') {
    const demoResponse = getDemoApiResponse(url, String(fetchOptions.method ?? 'GET'));
    if (demoResponse !== undefined) return demoResponse as T;
  }
  ...
  try {
    ...
    throw new ApiError(ApiErrorCode.NETWORK_ERROR, `Network Error: ${errorMessage}`, undefined, {
  ...
  if (!response.ok) {
    ...
    if (response.status === 401) {
      ...
      throw new ApiError(ApiErrorCode.UNAUTHORIZED, 'Unauthorized - please sign in again', 401);
    }
    ...
    toast.error('API Request Failed', {
      ...
      cancel: {
        ...
        onClick: () => {
          navigator.clipboard.writeText(
            JSON.stringify({ endpoint: url, status: response.status, error: errorData }, null, 2),
          );
          toast.success('Error copied to clipboard', { duration: 2000 });
        },
      },
