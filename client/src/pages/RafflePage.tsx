...
const RafflePage = () => {
  ...
  useEffect(() => {
    setIsPrizeClaimed(Boolean(raffle?.winningsWithdrawn));
  }, [raffle?.winningsWithdrawn]);
  ...
  const handleIncrement = () =>
    setTicketCount((prev) => Math.min(prev + 1, maxTickets - entries));
  ...
  const isWinner = Boolean(
    normalizedAddress &&
    normalizedWinner &&
    normalizedAddress === normalizedWinner,
  );
  const canClaimPrize =
    raffle.status === "finalized" && isWinner && !isPrizeClaimed;
  ...
  const handleTicketPurchase = async () => {
    ...
    try {
      ...
      const result = await ContractService.buyTickets({
        raffleId,
        ticketCount,
        maxPricePerTicket: String(BigInt(Math.ceil(parseFloat(raffle.ticketPrice) * 10_000_000_000))),
      });
  ...
  return (
    <div className="w-full mx-auto max-w-7xl px-6 md:px-12 lg:px-16 py-8 flex flex-col space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <Helmet>
        ...
        <meta
          property="og:image"
          content={image || `${window.location.origin}/og-image.png`}
        />
      ...
      {raffle.status === "finalized" && isWinner && (
        <RaffleWinnerBanner isWinner />
      )}
            ...
            {isActive ? (
              <div className="space-y-4">
                ...
                <button
                  ...
                  style={{
                    background:
                      "linear-gradient(100.92deg, #FE3796 13.57%, #3931F9 97.65%)",
                  }}
              ...
              <div className="space-y-4">
                {isFinalized && winner ? (
                  <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-2xl text-center space-y-2">
                    ...
                    <p className="text-xs text-yellow-500/80 font-bold uppercase">
                      {isWinner ? "You Won!" : t("raffle.winner")}
                    </p>
                    <p className="text-sm font-black text-gray-900 dark:text-white truncate px-2">
                      {winner}
                    </p>
                  ...
                  <div className="p-4 bg-gray-500/10 border border-gray-500/20 rounded-2xl text-center">
                    ...
                    <p className="text-xs text-gray-500">
                      {t("raffle.noWinnerYet")}
                    </p>
                  </div>
                )}

                {canClaimPrize || isPrizeClaimed ? (
                  <div className="p-4 rounded-2xl border border-pink-500/20 bg-pink-500/10 space-y-4">
                    <div className="text-center space-y-1">
                      <p className="text-xs font-bold uppercase tracking-widest text-pink-600 dark:text-[#FE3796]">
                        You Won!
                      </p>
                    ...
                    <button
                      ...
                      disabled={isClaimingPrize || isPrizeClaimed}
                      className="w-full py-4 rounded-xl font-black text-gray-900 dark:text-white tracking-wider shadow-lg shadow-[#FE3796]/20 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
                      style={{
                        background:
                          "linear-gradient(100.92deg, #FE3796 13.57%, #3931F9 97.65%)",
                      }}
          ...
          <div className="bg-white dark:bg-[#11172E]/50 border border-gray-200 dark:border-white/5 rounded-3xl p-6 flex items-start space-x-4">
            ...
            <div className="space-y-1">
              <p className="text-sm font-bold text-gray-900 dark:text-white">
                {t("raffle.provablyFair")}
              </p>
              <p className="text-xs text-gray-500 leading-relaxed">
                {t("raffle.fairnessDetail")}
              </p>
            </div>
          </div>
            ...
            <NotificationSubscribeButton
              ...
              onAuthRequired={() =>
                toast.info("Sign in required", {
                  description:
                    "Connect your wallet and sign in to subscribe to notifications.",
                })
              }
            />
          </div>
