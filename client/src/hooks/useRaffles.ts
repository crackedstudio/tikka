...
export const useRaffles = (filters?: RaffleListFilters) => {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    setError(null);
    // We don't necessarily want to clear all raffles while loading new ones for pagination/filters
    // so we'll leave setRaffles(null) out here if it's not desired.
    // But the error was because I used the wrong setter name.

    const parsed = serializedFilters
      ? (JSON.parse(serializedFilters) as RaffleListFilters)
      : undefined;
    fetchRaffles(parsed)
      .then((response) => {
...
export const useRaffle = (raffleId: number) => {
  ...
  useEffect(() => {
    ...
    const currentRequest = ++requestId.current;
        ...
        setIsLoading(true);
        setError(null);
        setRaffle(null); // Clear previous raffle while loading new one
    ...
    fetchUserHistory(address, HISTORY_PAGE_SIZE, page * HISTORY_PAGE_SIZE)
      ...
      .catch((err) => {
        ...
        setError(
          err instanceof Error
            ? err
            : new Error("Failed to fetch user history"),
        );
      })
      .finally(() => {
        if (currentRequest !== requestId.current) return;
