...
describe("useRaffles", () => {
  ...
  it("fetches and returns raffles on mount", async () => {
    vi.spyOn(raffleService, "fetchRaffles").mockResolvedValue(
      mockRaffleListResponse,
    );
  ...
  it("sets error when fetch fails", async () => {
    ...
    vi.spyOn(raffleService, "fetchRaffles").mockRejectedValue(
  ...
  it("refetch triggers a new fetch", async () => {
    const spy = vi
      ...
      .mockResolvedValue(mockRaffleListResponse);
  ...
  it("cancels stale requests when filters change", async () => {
    ...
    const { result, rerender } = renderHook(
      ({ filters }: { filters?: RaffleListFilters }) => useRaffles(filters),
...
describe("useRaffle", () => {
  ...
  it("fetches and returns raffle detail", async () => {
    vi.spyOn(raffleService, "fetchRaffleDetail").mockResolvedValue(
      mockRaffleDetail,
    );
  ...
  it("refetch triggers a new fetch", async () => {
    const spy = vi
      ...
      .mockResolvedValue(mockRaffleDetail);
    vi.spyOn(raffleService, "mapDetailToFormattedRaffle").mockReturnValue(
      mockFormattedRaffle,
    );
  ...
  it("cancels stale requests when raffleId changes", async () => {
    ...
    vi.spyOn(raffleService, "mapDetailToFormattedRaffle").mockImplementation(
      (detail) => {
        if (detail.id === 1) return firstFormatted;
        if (detail.id === 2) return secondFormatted;
        return mockFormattedRaffle;
      },
    );
...
describe("useUserHistory", () => {
  ...
  it("fetches and returns history items", async () => {
    vi.spyOn(raffleService, "fetchUserHistory").mockResolvedValue(
      mockHistoryResponse,
    );
