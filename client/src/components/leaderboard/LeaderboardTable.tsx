...
const LeaderboardTable: React.FC<LeaderboardTableProps> = ({ players }) => {
  ...
  return (
    <div
      className="bg-white dark:bg-[#1E1932] rounded-xl overflow-hidden"
      role="table"
      aria-label="Leaderboard"
    >
      {/* Table Header */}
      <div
        className="grid grid-cols-4 gap-4 px-6 py-4 bg-gray-200 dark:bg-[#2A264A] text-gray-700 dark:text-gray-300 text-sm font-medium"
        role="rowgroup"
      >
        <div role="columnheader">Rank</div>
        <div role="columnheader">Player</div>
        <div role="columnheader">Wins</div>
        <div role="columnheader">ETH won</div>
      </div>

      {/* Table Rows */}
      <div className="divide-y divide-gray-700" role="rowgroup">
        {players.map((player) => (
          <div
            key={player.id}
          ...
          >
            ...
            <div className="flex items-center" role="cell">
              <span
                ...
                aria-label={getRankLabel(player.rank)}
              >
                {player.rank}
              </span>
            </div>
