...
const typesSource = fileURLToPath(new URL('../packages/types/src/index.ts', import.meta.url));
const typesSubpathAliases = ['contract', 'events', 'raffle', 'ticket', 'user'].map((subpath) => ({
  find: `@tikka/types/${subpath}`,
  replacement: fileURLToPath(new URL(`../packages/types/src/${subpath}.ts`, import.meta.url)),
}));
