...
const smokeRaffleDetails = getDemoRaffleByName('live');
...
test.describe('E2E smoke tests', () => {
  ...
  test('landing page navigates to raffle details', async ({ page }) => {
    ...
    const cardLink = page.locator(`a[href="/raffles/${smokeRaffleDetails.id}"]`).first();
    await expect(cardLink).toBeVisible({ timeout: 10000 });
    await cardLink.click();

    await expect(page).toHaveURL(new RegExp(`/raffles/${smokeRaffleDetails.id}$`));
    await expect(page.getByText(smokeRaffleDetails.title!)).toBeVisible();
  });
  ...
  test('create raffle happy path shows created success modal', async ({ page }) => {
    ...
    await page.fill(
      ...
      'A smoke draft raffle created by e2e.',
    );
