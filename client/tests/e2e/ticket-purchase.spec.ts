...
async function gotoHome(page: Page) {
  ...
  await expect(page.getByTestId('enter-raffle-btn').first()).toBeVisible({ timeout: 10_000 });
}
...
test.describe('Ticket purchase flow', () => {
  ...
  test.describe('button loading state', () => {
    ...
    test('re-enables the button after the purchase completes', async ({ page }) => {
      await gotoHome(page);
      const btn = page.getByTestId('enter-raffle-btn').first();
      await btn.click();
  ...
  test.describe('success modal', () => {
    test('success modal appears with "Let\'s go!!!" heading', async ({ page }) => {
      await gotoHome(page);
    ...
    test('Close (×) button dismisses the success modal', async ({ page }) => {
      await gotoHome(page);
      await clickFirstEnterButton(page);
      await waitForSuccessModal(page);
    ...
    test('clicking the backdrop closes the success modal', async ({ page }) => {
      await gotoHome(page);
      await clickFirstEnterButton(page);
      await waitForSuccessModal(page);
  ...
  test.describe('ticket count update', () => {
    test('entries count increments by 1 after a successful purchase', async ({ page }) => {
      await gotoHome(page);
    ...
    test('entries count increments again on a second purchase', async ({ page }) => {
      await gotoHome(page);
      const entriesEl = page.getByTestId('entries-count').first();
      const before = Number(await entriesEl.innerText());
  ...
  test.describe('navigation', () => {
    test('clicking the raffle card link navigates to the detail page', async ({ page }) => {
      await gotoHome(page);
