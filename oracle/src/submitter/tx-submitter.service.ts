...
export class TxSubmitterService {
  ...
  async submitRandomnessTyped(
    ...
    try {
      ...
      for (let attempt = 1; attempt <= this.maxAttempts; attempt++) {
        ...
        try {
          telemetry.currentState = TransactionState.BUILDING;
          const prepared = await this.txBuilder.buildPreparedTx(
            this.rpcServer,
            this.contractId,
            this.networkPassphrase,
            publicKey,
          ...
          const submitResult = await this.submissionService.submitTransactionWithRetry(
            this.rpcServer,
            prepared,
            telemetry,
            this.errorClassifier,
            this.logTelemetry.bind(this),
          );
          ...
          if (submitResult.outcome) {
            ...
            if (submitResult.outcome.status === 'SUCCESS') {
              ...
              this.feeStrategy.recordSubmissionFailure(
                raffleId,
                'PRNG',
                submitResult.outcome.error,
              );
            }
          ...
          if (submitResult.shouldRetry) {
            if (submitResult.bumpFee) {
              ...
              if (feeBump > prevBump)
                this.feeStrategy.recordFeeBump(telemetry.raffleId || 0, 'PRNG', feeBump);
              this.logTelemetry(telemetry, `Bumping fee multiplier to ${feeBump}x`);
            }
            await this.logRetryAndBackoff(
              'submitRandomnessTyped',
              attempt,
              new Error('Retry requested'),
            );
          } else {
            const outcome = this.errorClassifier.classifyError(
              new Error('Unknown non-retriable error'),
  ...
  async submitRandomness(raffleId: number, randomness: RandomnessResult): Promise<SubmitResult> {
    ...
    if (outcome.status === 'SUCCESS' || outcome.status === 'DUPLICATE_SUCCESS') {
      return {
        ...
        success: true,
        feePaid: 'feePaid' in outcome ? outcome.feePaid : undefined,
      };
    }
    return {
      txHash: 'txHash' in outcome && outcome.txHash ? outcome.txHash : '',
      ledger: 0,
      success: false,
    };
  }
  ...
  async submitCommitment(raffleId: number, commitment: string): Promise<SubmitResult> {
    try {
      const publicKey = await this.keyService.getPublicKey();
      const prepared = await this.txBuilder.buildCommitmentTx(
        this.rpcServer,
        this.contractId,
        this.networkPassphrase,
        publicKey,
      ...
      return {
        txHash,
        ledger: (confirm.ledger as number) || 0,
        success: confirm?.status === 'SUCCESS',
      };
    } catch (e: any) {
  ...
  async submitReveal(raffleId: number, secret: string, nonce: string): Promise<SubmitResult> {
    try {
      const publicKey = await this.keyService.getPublicKey();
      const prepared = await this.txBuilder.buildRevealTx(
        this.rpcServer,
        this.contractId,
        this.networkPassphrase,
        publicKey,
  ...
  private createFatalOutcome(error: any, telemetry: TelemetryContext): TransactionOutcome {
    ...
    this.logTelemetry(
      { ...telemetry, finalOutcome: TransactionState.FAILED },
      `Fatal unexpected error: ${errorMessage}`,
    );
    return {
      status: 'FAILED',
      txHash: telemetry.txHash,
  ...
  private async logRetryAndBackoff(
    operation: string,
    attempt: number,
    reason: unknown,
  ): Promise<void> {
  ...
  private logTelemetry(telemetry: TelemetryContext, message: string): void {
    ...
    const logLevel =
      finalOutcome === TransactionState.FAILED
        ? 'error'
        : finalOutcome === TransactionState.TIMEOUT ||
            finalOutcome === TransactionState.NETWORK_ERROR
          ? 'warn'
          : 'log';
    const enrichedFields: OracleLogFields = {
      ...fields,
      outcome: finalOutcome?.toString() || currentState?.toString() || 'unknown',
      context: TxSubmitterService.name,
    };
    if (logLevel === 'error') {
      this.logger.error(
        `[TX-${telemetry.raffleId}-${telemetry.attempt}] ${message}`,
