...
jest.mock(
  '@albedo-link/intent',
  () => {
    throw new Error('@albedo-link/intent must not be evaluated while importing an SDK entry point');
  },
  { virtual: true },
);

import { WalletAdapter, WalletAvailabilityCode, WalletName } from '../wallet/wallet.interface';
import {
  AlbedoAdapter,
  FreighterAdapter,
  LobstrAdapter,
