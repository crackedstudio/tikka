...
import { ContractErrorType as TypesContractErrorType } from '@tikka/types';
...
describe('typed contract errors', () => {
  it.each([
    ...
    [InsufficientFundsError, ContractErrorType.INSUFFICIENT_FUNDS, 'low balance'],
    [UnauthorizedError, ContractErrorType.UNAUTHORIZED, 'no access'],
  ])('creates %p with the expected contract code', (ErrorType, code, message) => {
  ...
  it('maps known Soroban contract codes to typed errors', () => {
    expect(toTypedContractError('failed', 'Error(Contract, #1)')).toBeInstanceOf(
      RaffleNotFoundError,
    );
    expect(toTypedContractError('failed', 'Error(Contract, #3)')).toBeInstanceOf(RaffleFullError);
    expect(toTypedContractError('failed', 'Error(Contract, #4)')).toBeInstanceOf(
      InsufficientFundsError,
    );
    expect(toTypedContractError('failed', 'Error(Contract, #5)')).toBeInstanceOf(UnauthorizedError);
    expect(toTypedContractError('failed', 'Error(Contract, #35)')).toBeInstanceOf(RaffleEndedError);
  });

  it('wraps generic TikkaSdkError contract failures into typed errors when possible', () => {
    const wrapped = toTypedSdkError(
      new TikkaSdkError(TikkaSdkErrorCode.ContractError, 'buy failed', 'Error(Contract, #35)'),
    );
...
describe('RpcError', () => {
  ...
  describe('fromResponse()', () => {
    it('builds an RpcError from a response object', () => {
      const err = RpcError.fromResponse('https://rpc.example.com', 'simulateTransaction', {
        status: 503,
...
describe('ContractErrorType alignment with @tikka/types', () => {
  it('maps ContractErrorType.RAFFLE_ENDED to TikkaSdkErrorCode.RaffleEnded', () => {
    expect(TypesContractErrorType.RAFFLE_ENDED).toBe(TikkaSdkErrorCode.RaffleEnded);
  });

  it('maps ContractErrorType.RAFFLE_FULL to TikkaSdkErrorCode.RaffleFull', () => {
    expect(TypesContractErrorType.RAFFLE_FULL).toBe(TikkaSdkErrorCode.RaffleFull);
  });

  it('maps ContractErrorType.INSUFFICIENT_FUNDS to TikkaSdkErrorCode.InsufficientFunds', () => {
    expect(TypesContractErrorType.INSUFFICIENT_FUNDS).toBe(TikkaSdkErrorCode.InsufficientFunds);
  });

  it('maps ContractErrorType.RAFFLE_NOT_FOUND to TikkaSdkErrorCode.RaffleNotFound', () => {
    expect(TypesContractErrorType.RAFFLE_NOT_FOUND).toBe(TikkaSdkErrorCode.RaffleNotFound);
  });

  it('maps ContractErrorType.UNAUTHORIZED to TikkaSdkErrorCode.Unauthorized', () => {
    expect(TypesContractErrorType.UNAUTHORIZED).toBe(TikkaSdkErrorCode.Unauthorized);
  });

  it('CONTRACT_ERROR_CODE_MAP contains all ContractErrorType entries', () => {
    const errorClasses = [
      RaffleNotFoundError,
      RaffleEndedError,
      RaffleFullError,
    ...
    errorClasses.forEach((ErrorClass) => {
      expect(new ErrorClass('test')).toBeInstanceOf(TikkaSdkError);
    });
  });
});
